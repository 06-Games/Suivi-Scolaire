...
public static class NativeShare
{
    ...
    public static void SharePC(string[] filePath, string title, string directory, string[] fileName, ExtensionFilter[] filters)
    {
        ...
        {
            string path = StandaloneFileBrowser.SaveFilePanel(title, directory, 
                fileName?.GetValue(0).ToString() ?? Path.GetFileName(filePath[0]), 
                filters ?? new ExtensionFilter[] { new ExtensionFilter("", Path.GetExtension(fileName[0])) }
            );
            if (!string.IsNullOrEmpty(path)) File.Copy(filePath[0], path, true);
        }
